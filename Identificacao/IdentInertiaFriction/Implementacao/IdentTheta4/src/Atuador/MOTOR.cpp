...
void Motor::setTrimForceinNewtons(float force){

	trimForceinNewton = force;
}

void Motor::reachTrimForceinNewton(float force){

	if (force > forceNewtonMax)
	{
		cout<< "Saturação superior"<< " Forca maxima aceitavel e de "<< forceNewtonMax <<endl;
		force = forceNewtonMax;
	}
